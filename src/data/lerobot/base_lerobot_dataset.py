...
def resolve_image_obs_indices(
    obs_size: int,
    image_obs_indices: Optional[Sequence[int]],
) -> tuple[int, ...]:
    if image_obs_indices is None:
        return tuple(range(obs_size))
    resolved = tuple(int(index) for index in image_obs_indices)
    if not resolved:
        raise ValueError("`image_obs_indices` must contain at least one index.")
    if tuple(sorted(set(resolved))) != resolved:
        raise ValueError(
            "`image_obs_indices` must be strictly increasing and contain no duplicates, "
            f"got {resolved}."
        )
    if resolved[0] < 0 or resolved[-1] >= obs_size:
        raise ValueError(
            f"`image_obs_indices` must be within [0, {obs_size}), got {resolved}."
        )
    return resolved
    ...
    def __init__(
        ...
        for meta in self.image_meta:
            ...
            delta_timestamps[meta["lerobot_key"]] = [
                ((index - past_obs_size) * global_sample_stride) / fps
                for index in self.image_obs_indices
            ]
