...
from experiments.robodojo.batched_inference import DynamicInferenceBatcher
...
def _resolve_sim_cfg_name(sim_cfg_path: Optional[str], sim_cfg_name: Optional[str]) -> str:
    ...
    if _is_none_like(sim_cfg_name):
        return "benchmark/sim_robodojo.yaml"
...
def action_vector_to_xpolicylab(action: np.ndarray) -> Dict[str, Any]:
    ...
    if vector.shape != (14,):
        raise ValueError(f"Expected a 14-D RoboDojo action, got shape {vector.shape}.")
    return {
...
class WorldActionRoboDojoPolicy:
    def __init__(
        self,
        ...
        logger.info(
            "Initialized WorldActionRoboDojoPolicy | ckpt=%s | stats=%s | horizon=%d | replan=%d",
            checkpoint_path,
            dataset_stats_path,
            self.action_horizon,
            self.replan_steps,
    ...
    def _build_robodojo_image_tensor(self, observation: Dict[str, Any]) -> torch.Tensor:
    ...
    def _infer_action_chunk(
        ...
        image_tensor = self._build_robodojo_image_tensor(observation)
...
def get_model(usr_args: Dict[str, Any]):
    ...
    policy = WorldActionRoboDojoPolicy(
        model_cfg=cfg.model,
