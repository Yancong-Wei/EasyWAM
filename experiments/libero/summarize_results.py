...
from experiments.libero.result_utils import valid_result_path
    ...
    for suite in ["libero_spatial", "libero_object", "libero_goal", "libero_10", "libero_90"]:
        ...
        for filename in sorted(os.listdir(suite_dir)):
            ...
            try:
                ...
                continue
            result_path = valid_result_path(Path(output_dir), suite, task_id, expected_episodes=None)
            if result_path is None or result_path.name != filename or task_id in seen_task_ids:
                continue
