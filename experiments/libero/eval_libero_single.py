...
def _get_video_mode(cfg: DictConfig) -> str:
    mode = str(cfg.EVALUATION.get("video_mode", "none")).strip().lower()
    if mode not in {"none", "failures", "all"}:
        raise ValueError(
            f"Unsupported EVALUATION.video_mode: {mode}. "
            "Expected one of: ['none', 'failures', 'all']."
        )
    return mode
...
def run_single_episode(
    ...
    *,
    ...
    visualize_future_video = bool(cfg.EVALUATION.get("visualize_future_video", False))
    collect_rollout_video = _get_video_mode(cfg) != "none"
    capture_steps = set(_get_future_frame_capture_steps(cfg)[1:])

    sim_started = time.perf_counter()
    env.reset()
    obs = env.set_init_state(initial_state)
    timing["simulation_seconds"] += time.perf_counter() - sim_started
    ...
    pbar = tqdm(
        ...
        disable=not bool(cfg.EVALUATION.get("progress", True)),
    )
...
def run_single_task(
    ...
    *,
    ...
    visualize_future_video = bool(cfg.EVALUATION.get("visualize_future_video", False))
    video_mode = _get_video_mode(cfg)
    results = {
        "successes": 0,
        "failure_episodes": [],
        "success_episodes": [],
...
def build_eval_runtime(cfg: DictConfig) -> LiberoEvalRuntime:
    ...
    if env_num != 1:
        raise ValueError("Only env_num=1 is supported by the LIBERO evaluator.")
...
def eval_single_process(cfg: DictConfig):
    ...
    results = evaluate_task_with_runtime(cfg, runtime)
